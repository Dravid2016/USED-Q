...
const Beams = ({
  ...
  const mouseState = useRef({ currentX: 0, currentY: 0, targetX: 0, targetY: 0 });
...
const BeamsScene = ({
  ...
  useFrame((_, delta) => {
    ...
    const lerpFactor = Math.min(0.08, delta * 5);
    mouseState.current.currentX += (mouseState.current.targetX - mouseState.current.currentX) * lerpFactor;
    mouseState.current.currentY += (mouseState.current.targetY - mouseState.current.currentY) * lerpFactor;

    // Reactively move directional light based on cursor position
    if (dirLightRef.current) {
      dirLightRef.current.position.x = mouseState.current.currentX * 14;
      dirLightRef.current.position.y = 3 + mouseState.current.currentY * 8;
    }

    // Smoothly tilt beams group according to cursor motion
    if (groupRef.current) {
      groupRef.current.rotation.z = degToRad(rotation) + mouseState.current.currentX * 0.12;
      groupRef.current.rotation.x = mouseState.current.currentY * 0.08;
    }
  });
  ...
  return (
    <>
      <group ref={groupRef} rotation={[0, 0, degToRad(rotation)]}>
        ...
        <DirLight ref={dirLightRef} color={lightColor} position={[0, 3, 10]} />
      </group>
...
const MergedPlanes = forwardRef<THREE.Mesh, any>(({ material, width, count, height }, ref) => {
...
const DirLight = forwardRef<THREE.DirectionalLight, { position: [number, number, number]; color: string }>(({ position, color }, ref) => {
  const dir = useRef<THREE.DirectionalLight>(null!);
  useImperativeHandle(ref, () => dir.current);
