...
export default function Header({ searchQuery = '', dark = false, transparent = false, hideSearch = false }: HeaderProps) {
  const headerBgClass = transparent
    ...
    : dark
    ? 'bg-[#0B0B0D] text-white border-b border-gray-800 shadow-sm'
    : 'bg-white border-b border-[#E7E7E3] text-[#12151A] shadow-sm';
      ...
      <div className="max-w-[1320px] mx-auto px-4 lg:px-6">
        <div className="flex items-center gap-3.5 h-16">
          ...
          <nav className="hidden md:flex items-center gap-1">
            <Link to="/saved" className={`flex flex-col items-center gap-0.5 px-3.5 py-1.5 rounded-full transition-colors ${dark ? 'hover:bg-white/10 text-gray-300 hover:text-white' : 'hover:bg-black/5 text-[#12151A]'}`}>
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
              </svg>
              <span className="text-[11px] font-semibold">Saved</span>
            </Link>
            <Link to="/messages" className={`relative flex flex-col items-center gap-0.5 px-3.5 py-1.5 rounded-full transition-colors ${dark ? 'hover:bg-white/10 text-gray-300 hover:text-white' : 'hover:bg-black/5 text-[#12151A]'}`}>
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <Link to="/notifications" className={`relative flex flex-col items-center gap-0.5 px-3.5 py-1.5 rounded-full transition-colors ${dark ? 'hover:bg-white/10 text-gray-300 hover:text-white' : 'hover:bg-black/5 text-[#12151A]'}`}>
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <Link to="/profile" className={`flex flex-col items-center gap-0.5 px-3.5 py-1.5 rounded-full transition-colors ${dark ? 'hover:bg-white/10 text-gray-300 hover:text-white' : 'hover:bg-black/5 text-[#12151A]'}`}>
          ...
          <Link
            to="/sell"
            className="hidden md:flex items-center gap-1.5 brand-gradient text-[#12151A] font-extrabold text-sm px-6 py-2.5 rounded-full hover:opacity-90 transition-opacity shrink-0 shadow-sm"
          >
